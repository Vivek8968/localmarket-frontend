...
// Mock data
const mockShops: { [key: string]: Shop } = {
  '1': {
    id: '1',
    ...
    rating: 4.5,
    isOpen: true,
    image: 'https://via.placeholder.com/800x400?text=TechWorld+Electronics'
  },
  '2': {
    id: '2',
    name: 'Battery Hub',
    ...
    rating: 4.3,
    isOpen: true,
    image: 'https://via.placeholder.com/800x400?text=Battery+Hub'
  }
};
...
export default function ShopPage() {
  ...
  const loadShopData = async (id: string) => {
    setLoading(true);
    try {
      // In production: const shopData = await api.getShopById(id);
      const shopData = mockShops[id];
      setShop(shopData);

      // In production: const productsData = await api.getProductsByShop(id);
      const productsData = mockProducts[id] || [];
